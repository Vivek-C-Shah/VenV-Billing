...
const Invoices = () => {
  ...
  const openPaymentHistory = (id) => {
    history.push(`/invoice/${id}/payment-history`);
  };
  ...
  const filteredRows = rows.filter((row) =>
    row.client.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    return (
      ...
      >
        ...
        <p style={{ padding: "40px", color: "gray", textAlign: "center" }}>
          No invoice yet. Click the plus icon to create an invoice
        </p>
      </div>
    );
  }
