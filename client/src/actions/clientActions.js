...
export const getClientInvoiceSummary = (id) => async (dispatch) => {
  try {
    console.log(id + "error");
    dispatch({ type: START_LOADING });
    const {
      data: { data },
    } = await api.fetchClientInvoiceSummary(id);
    dispatch({ type: FETCH_CLIENT_INVOICES_SUMMARY, payload: data });
    dispatch({ type: END_LOADING });
  } catch (error) {
    console.log(error);
  }
...
export const postBankTransfer =
  (formData, openSnackbar) => async (dispatch) => {
    try {
      const { data } = await api.postBankTransfer(formData);
      dispatch({ type: POST_BANK_TRANSFER, payload: data });
      openSnackbar("Cash transfer successful");
    } catch (error) {
      console.log(error);
    }
  };
